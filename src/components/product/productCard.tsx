...
export const ProductCard = ({
  ...
  return (
    <div
      className={`product-card overflow-hidden h-full flex-col items-start justify-center ${variant === 'small' ? 'w-36' : 'w-50'
        } ${className ?? ' '}`}
      {...props}
    >
      <div className="w-full inline-flex items-center justify-start">
        <Image
          src={imageUrl}
          alt={title}
          className={`w-full shrink-0object-cover ${variant === 'small' ? 'h-36' : 'h-50'
            }`}
        />
      </div>
      ...
      <Link href='/' className={`w-full flex flex-col h-full grow items-start justify-start p-2 ${variant === 'small' ? 'px-2 py-1' : 'px-2 py-1'
        ...
        <div className="w-full inline-flex items-end justify-between pt-1 pb-1">
          <span className="contrast-text text-xs ">
...
export const ProductCardList = ({
  ...
  return(
    <section className="w-full max-w-[1440px] px-1 relative inline-flex justify-between items-start">
      <div data-color="Dark" className="flex-1 max-w-[1432px] pb-8 bg-[var(--page-bg)] inline-flex flex-col justify-start items-start gap-2.5 overflow-hidden">
        <div className="self-stretch px-2 py-6 bg-[var(--elements-bg)] flex flex-col justify-start items-start gap-2">
          ...
          <div className="self-stretch px-1 flex flex-col justify-start items-start gap-2.5 overflow-x-scroll scroll-auto">
            <div className="self-stretch inline-flex justify-start items-start gap-2 w-fit">
              {
                cards.map((product) => (
                  <ProductCard
                    key={product.id}
